class RentalPropertiesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @rental_property.update(rental_property_params)
        format.html { redirect_to @rental_property, notice: "Rental property was successfully updated." }
  ...
  private
    ...
    def rental_property_params
      params.require(:rental_property).permit(:name, :rent, :address, :age, :note, :nearest_station_id)
    end
end
